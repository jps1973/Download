		...
		if( RegisterClassEx( &wcMain ) )
		{
			...
			if( hWndMain )
			{
				...
				if( lpszArgumentList )
				{
					...
					if( nArgumentCount > DOWNLOAD_TAG_NAME_ARGUMENT )
					{
						// Tag name argument is valid

						// Get wide argument length
						nWideArgumentLength = lstrlenW( lpszArgumentList[ DOWNLOAD_TAG_NAME_ARGUMENT ] );

						// Get size required for argument
						nSizeNeeded = WideCharToMultiByte( CP_UTF8, 0, lpszArgumentList[ DOWNLOAD_TAG_NAME_ARGUMENT ], nWideArgumentLength, NULL, 0, NULL, NULL );

						// Convert argument to ansi
						WideCharToMultiByte( CP_UTF8, 0, lpszArgumentList[ DOWNLOAD_TAG_NAME_ARGUMENT ], nWideArgumentLength, lpszArgument, nSizeNeeded, NULL, NULL );
						...
						if( nArgumentCount > DOWNLOAD_ATTRIBUTE_NAME_ARGUMENT )
						{
							// Attribute name argument is valid

							// Get wide argument length
							nWideArgumentLength = lstrlenW( lpszArgumentList[ DOWNLOAD_ATTRIBUTE_NAME_ARGUMENT ] );

							// Get size required for argument
							nSizeNeeded = WideCharToMultiByte( CP_UTF8, 0, lpszArgumentList[ DOWNLOAD_ATTRIBUTE_NAME_ARGUMENT ], nWideArgumentLength, NULL, 0, NULL, NULL );

							// Convert argument to ansi
							WideCharToMultiByte( CP_UTF8, 0, lpszArgumentList[ DOWNLOAD_ATTRIBUTE_NAME_ARGUMENT ], nWideArgumentLength, lpszArgument, nSizeNeeded, NULL, NULL );
							...
							if( nArgumentCount > DOWNLOAD_ATTRIBUTE_MUST_CONTAIN_ARGUMENT )
							{
								// Attribute must contain argument is valid

								// Get wide argument length
								nWideArgumentLength = lstrlenW( lpszArgumentList[ DOWNLOAD_ATTRIBUTE_MUST_CONTAIN_ARGUMENT ] );

								// Get size required for argument
								nSizeNeeded = WideCharToMultiByte( CP_UTF8, 0, lpszArgumentList[ DOWNLOAD_ATTRIBUTE_MUST_CONTAIN_ARGUMENT ], nWideArgumentLength, NULL, 0, NULL, NULL );

								// Convert argument to ansi
								WideCharToMultiByte( CP_UTF8, 0, lpszArgumentList[ DOWNLOAD_ATTRIBUTE_MUST_CONTAIN_ARGUMENT ], nWideArgumentLength, lpszArgument, nSizeNeeded, NULL, NULL );
